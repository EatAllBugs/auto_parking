...
bool RunReverseVerticleParking(const std::vector<LineSegment2d> &boundaries,
  ...
  for (double factor : {0.7, 0.6, 0.5, 0.4}) {
    thetas.push_back(
        NormalizeAngle(start.theta + factor * (0.5 * M_PI - start.theta)));
  }
  if (start.pos.x() < 0.0) {
    pose_candidates = get_pose_candidates({0.2, 0.4, 0.8}, {4.3, 4.6, 4.9},
                                          std::move(thetas));
  } else {
    pose_candidates = get_pose_candidates({-0.2, -0.4, -0.8}, {4.3, 4.6, 4.9},
                                          std::move(thetas));
  ...
  for (const auto &pose : pose_candidates) {
    ...
    for (double s = 0.3; s < 3.0; s += 0.3) {
      ...
      const Pose end =
          GetPoseAlongArc(first_path.ends.back(), MAX_KAPPA, s, type);
      LineCirclePath forward_path = {
          {first_path.ends.back()}, {s}, {type}, {MAX_KAPPA}, {end}};
  ...
  for (const auto &pose : pose_candidates) {
    ...
    for (double s = 0.0; s < 3.0; s += 0.3) {
      const Pose end =
          GetPoseAlongArc(first_path.ends.back(), MAX_KAPPA, s, type);
      Box2d av_box(end.pos + 0.5 * (FRONT_TO_RAC - REAR_TO_RAC) * end.dir,
                   end.theta, VEHICLE_LENGTH, VEHICLE_WIDTH);
      furthest_s = s;
    ...
    furthest_s -= 0.03;
    const Pose end1 =
        GetPoseAlongArc(first_path.ends.back(), MAX_KAPPA, furthest_s, type);
    ...
    for (double s1 = -0.3; s1 > -furthest_s - 0.5; s1 -= 0.2) {
      ...
      Box2d av_box(end2.pos + 0.5 * (FRONT_TO_RAC - REAR_TO_RAC) * end2.dir,
                   end2.theta, VEHICLE_LENGTH, VEHICLE_WIDTH);
      if (has_collision(av_box)) {
        break;
      }
