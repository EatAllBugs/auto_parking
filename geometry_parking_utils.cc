...
Pose GetPoseAlongArc(const Pose &start, double kappa, double length,
                     char type) {
  ...
  const double x =
      start.pos.x() + delta_x * start.dir.x() - delta_y * start.dir.y();
  const double y =
      start.pos.y() + delta_x * start.dir.y() + delta_y * start.dir.x();
  const double theta =
      apollo::common::math::NormalizeAngle(start.theta + delta_theta);
  return {Vec2d(x, y), theta, Vec2d::CreateUnitVec2d(theta)};
}

std::vector<Pose> ConvertPathToDiscretePoses(const LineCirclePath &path,
                                             double step) {
  std::vector<Pose> res;
    ...
    switch (path.types[i]) {
      case 'S': {
        for (double s = step; s < std::abs(path.lengths[i]) + step; s += step) {
          ...
          const auto pos =
              cur_start.pos + std::copysign(s, path.lengths[i]) * cur_start.dir;
      ...
      case 'R': {
        for (double s = step; s < std::abs(path.lengths[i]) + step; s += step) {
          s = std::min(s, std::abs(path.lengths[i]));
          res.push_back(GetPoseAlongArc(cur_start, path.kappas[i],
                                        std::copysign(s, path.lengths[i]),
                                        path.types[i]));
        }
        break;
      }
...
bool HasOverlapWithPath(const LineCirclePath &path,
                        const std::vector<LineSegment2d> &boundaries,
                        double resolution) {
  const auto poses = ConvertPathToDiscretePoses(path, resolution);
  std::vector<Box2d> av_boxes;
  for (const auto &pose : poses) {
    av_boxes.emplace_back(
        pose.pos + 0.5 * (FRONT_TO_RAC - REAR_TO_RAC) * pose.dir, pose.theta,
        VEHICLE_LENGTH, VEHICLE_WIDTH);
  }
  for (const auto &boundary : boundaries) {
    for (const auto &box : av_boxes) {
      if (box.HasOverlap(boundary)) return true;
    }
  }
  return false;
}