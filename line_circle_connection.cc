...
bool LineCircleOrCircleLine(const Pose& start, const Pose& end,
                            double max_kappa, LineCirclePath* result) {
  ...
  if (l1 < l2) {
    ...
    Pose end1 = {start.pos + result->lengths[0] * start.dir, start.theta,
                 start.dir};
    Pose end2 = GetPoseAlongArc(end1, result->kappas[1], result->lengths[1],
                                result->types[1]);
    ...
    Pose end1 = GetPoseAlongArc(start, result->kappas[0], result->lengths[0],
                                result->types[0]);
    Pose end2 = {end1.pos + end1.dir * result->lengths[1], end1.theta,
                 end1.dir};
    result->ends = {end1, end2};
    return true;
  }
